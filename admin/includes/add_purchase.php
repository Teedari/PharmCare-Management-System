...
//calculate for total amount
    $t_amnt = $buy * $quant;
    if(is_codeExit($code)){
       echo "<div class='alert alert-info'><strong>Info!</strong> purchase code can be used once.</div>"; 
    }else{
...
if(!empty($batchNo) || !empty($code)){
         global $connection;
    if(stock_batchNoExit($batchNo)){
     if(checkCheck($batchNo,$medicine)){
        ...
        $query3 = "INSERT INTO purchase(purchaseCode,supName,paymentMethod,grandTotal,status)  VALUES ($code,'{$supplier}','{$payMethod}',$grandT,'{$status}')";
       $result3 = mysqli_query($connection,$query3);
         
     }
    }else{

       $query2 = "INSERT INTO products(name, batchNo, totalQuantity, sellPrice, buyPrice)  VALUES ('{$medicine}',{$batchNo},{$quant},'{$_SESSION['sell']}','{$_SESSION['buy']}')";
