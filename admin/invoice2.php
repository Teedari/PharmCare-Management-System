...
if(isset($_GET['p_code'])){
   $code = $_GET['p_code'];
    global $connection;
    $result = mysqli_query($connection,"SELECT * FROM salesList WHERE salesCode = {$code}");
    confirmQuery($result);
    while($data = mysqli_fetch_assoc($result)){
        $customer = $data['customer'];
        $id = $data['id'];
    }
$pdf->Cell(50,7,"$customer",0,0);
$pdf->Cell(89,7,'',0,0);
...
global $connection;
$query2 = "SELECT SUM(amount) AS sumBalance FROM salesList WHERE salesCode = {$code}";
